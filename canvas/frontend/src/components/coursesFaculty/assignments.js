...
submitGrade=()=>{
    (async()=>{
        let {sid,uid,aid}=this.props.studentassignment.submissionFiles[0];
...
/////pdf view functions
fileClicked=(index)=>{
    let file=this.props.studentassignment.submissionFiles[index].file.data;
    let filename=this.props.studentassignment.submissionFiles[index].filename;
    let grades=this.props.studentassignment.submissionFiles[index].grades;
    console.log(file);
           ...
           <tbody>
                 {
                            this.props.studentassignment.people.map((data,index)=>{
                                return <tr key={index} onClick={()=>{this.getSubmissionbyid(data.uid,data.aid)}}><td className="sub-name">{data.name}   </td><td>{data.grades}</td></tr>
                            })
                        }
                        </tbody>
                         </table>
